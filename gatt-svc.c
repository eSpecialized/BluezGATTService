...
/* Modified with parts from this post;
 *  https://stackoverflow.com/questions/5656530/how-to-use-shared-memory-with-linux-in-c
 *    see shakram02 response Mar 27 '16
 */
...
//new items
struct characteristic *maincharacteristic;
...
static const char *ias_alert_level_props[] = { "read", "write", "notify", NULL };
...
static bool chr_read(struct characteristic *chr, DBusMessageIter *iter)
{
    ...
    int i;
    for (i = 0; i < chr->vlen; i++)
    {
        printf(" %02x",chr->value[i]);
    }
    printf("\n");
						...
						const char *service_path)
{
    ...
    maincharacteristic = chr; //grab a pointer for use later.
    printf("maincharacteristic UUID = %s\n",chr_uuid);
...
void* runServerInThread(void *arg)
{
    g_main_loop_run(main_loop);
}

void runMainInputLoop(void)
{
    ...
    do {
        printf("type a start/stop/quit cmd:\n");
        fgets(inputString,buffSize,stdin);
        
        
        inputString[strlen(inputString) - 1]='\0'; //truncate it
        ...
        {
            chr_write(maincharacteristic, inputString,strlen(inputString));
        }
    } while (!__terminated);
}
...
int main(int argc, char *argv[])
{
    ...
    struct shmids_ds shmbuffer;
    int segment_size;
    const int shared_segment_size = 0x6400;
    //allocate shared memory
    int segmentID = shmget(IPC_PRIVATE, shared_segment_size,
                           IPC_CREAT | IPC_EXCL | S_IRUSR | S_IWUSR);
    
    //attach to the shared memory segment
    char *sharedmemory = (char*)shmat (segmentID,0,0);
    
    printf("shared memroy attached at address %p\n",sharedmemory);
    
    //check segment size
    shmcgtl(segmentID, IPC_STAT, &shmbuffer);
	...
	//g_main_loop_run(main_loop); //blocking call moved to runServerInThread
    int i = 0; //thread index tracker
    int err;
    
    err = pthread_create(&(tid[i]), NULL, &runServerInThread, NULL);
    
    if (err != 0)
    {
        fprintf(stderr, "Can't create a background server thread! %s\n",strerror(err));
    }
    else
    {
        runMainInputLoop();
